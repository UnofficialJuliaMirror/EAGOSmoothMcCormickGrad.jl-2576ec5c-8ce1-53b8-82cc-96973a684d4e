...
size of the decision space, and `optc` is a vector with stored values.
"""
function MCn_NewtonGS!(z_mc::Vector{SMCg{N,T}},x_mc::Vector{SMCg{N,T}},
                     YdH_mc::Union{Vector{SMCg{N,T}},SparseMatrixCSC{SMCg{N,T},Int64}},
                     YH_mc::Vector{SMCg{N,T}},nx::Int64) where {N,T<:AbstractFloat}

  S1::SMCg{N,T} = zero(SMCg{N,T})
  S2::SMCg{N,T} = zero(SMCg{N,T})
  S3::SMCg{N,T} = zero(SMCg{N,T})
  x_mc_int::Vector{SMCg{N,T}} = copy(x_mc)
  for i=1:nx
    S1 = zero(SMCg{N,T})
    S2 = zero(SMCg{N,T})
...
size of the decision space, and `optc` is a vector with stored values.
"""
function MCn_KrawczykCW!(z_mc::Vector{SMCg{N,T}},x_mc::Vector{SMCg{N,T}},
                        YdH_mc::Union{Vector{SMCg{N,T}},SparseMatrixCSC{SMCg{N,T},Int64}},
                        YH_mc::Vector{SMCg{N,T}},nx::Int64) where{N,T<:AbstractFloat}
  S1::SMCg{N,T} = zero(SMCg{N,T})
  x_mc_int::Vector{SMCg{N,T}} = copy(x_mc)
