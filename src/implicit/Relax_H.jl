...
function MC_impRelax(h,hj,p,pmid,X,P,mc_opts,param)
    ...
    x_mc = [SMCg(X[i].hi,X[i].lo,szero,szero,@interval(X[i].lo,X[i].hi),true,[∅],[1.0]) for i=1:nx]
    #x_mc = param[1]
    xa_mc = [SMCg{np,Float64}(X[i].lo,X[i].lo,szero,szero,@interval(X[i].lo,X[i].lo),true,[∅],[1.0]) for i=1:nx]
    xA_mc = [SMCg{np,Float64}(X[i].hi,X[i].hi,szero,szero,@interval(X[i].hi,X[i].hi),true,[∅],[1.0]) for i=1:nx]
    ...
    for k=1:mc_opts.kmax
      ...
      println("isolation point #3d: ", k)
      H_mc = h(z_mc,p_mc)
      dH_mc = hj(aff_mc,p_mc)
      println("isolation point #3e: ", k)
