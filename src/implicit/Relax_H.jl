...
function MC_impRelax(h::Function, hj::Function, p::Vector{SMCg{N,T}}, pmid::Vector{T},
    ...
    xA_mc::Vector{SMCg{np,T}} = [SMCg{np,T}(X[i].hi,X[i].hi,szero,szero,@interval(X[i].hi,X[i].hi),true,[∅],[1.0]) for i=1:nx]
    ...
    p_mc::Vector{SMCg{np,T}} = copy(p)
    pref_mc::Vector{SMCg{np,T}} = [SMCg{np,T}(pmid[i],pmid[i],sone,sone,@interval(P[i].lo,P[i].hi),false,[∅],[1.0]) for i=1:np]
    aff_mc::Vector{SMCg{np,T}} = [SMCg{np,Float64}(xA_mc[i].cc,xa_mc[i].cv,szero,szero,@interval(xa_mc[i].cv,xA_mc[i].cc),false,[∅],[1.0]) for i=1:nx]
...
"""
function MC_NimpRelax(h!::Function, hj!::Function, p::Vector{SMCg{N,T}}, pmid::Vector{T},
                     X::Vector{Interval{T}}, P::Vector{Interval{T}},
                     mc_opts::mc_opts{T},param::Vector{Vector{SMCg{N,T}}}) where {N,T<:AbstractFloat}
...
"""
function MC_NdimpRelax(h!::Function, hj!::Function, p::Vector{SMCg{N,T}}, pmid::Vector{T},
                     X::Vector{Interval{T}}, P::Vector{Interval{T}},
                     mc_opts::mc_opts{T},param::Vector{Vector{SMCg{N,T}}}) where {N,T<:AbstractFloat}
